...
// Defaults returns local-only, bounded defaults. Persistence credentials have
// no default and external telemetry is disabled.
func Defaults() Config {
	return Config{
		...
		Telemetry: TelemetryConfig{Mode: "noop", ServiceName: "thinkpixelmem"},
	}
}
...
func (c Config) Validate() error {
	...
	if c.HTTP.MaxBodyBytes < 1024 || c.HTTP.MaxBodyBytes > 4<<20 {
		return errors.New("http.max_body_bytes: must be between 1 KiB and 4 MiB")
	}
...
// Validate checks telemetry settings independently so the telemetry adapter
// can reject invalid programmatic configuration before allocating resources.
func (c TelemetryConfig) Validate() error {
	if c.Mode != "noop" && c.Mode != "otlp" {
		return errors.New("telemetry.mode: must be noop or otlp")
	}
	if c.ServiceName == "" || len(c.ServiceName) > 128 {

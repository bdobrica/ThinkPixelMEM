...
// Load applies defaults, an optional JSON file, TPMEM_ environment variables,
...
var environmentSetters = map[string]setter{
	"TPMEM_MODE":                              func(c *Config, v string) error { c.Mode = Mode(v); return nil },
	"TPMEM_HTTP_ADDRESS":                      func(c *Config, v string) error { c.HTTP.Address = v; return nil },
	"TPMEM_HTTP_READ_HEADER_TIMEOUT":          durationSetter("http.read_header_timeout", func(c *Config, d time.Duration) { c.HTTP.ReadHeaderTimeout = d }),
	"TPMEM_HTTP_READ_TIMEOUT":                 durationSetter("http.read_timeout", func(c *Config, d time.Duration) { c.HTTP.ReadTimeout = d }),
	"TPMEM_HTTP_WRITE_TIMEOUT":                durationSetter("http.write_timeout", func(c *Config, d time.Duration) { c.HTTP.WriteTimeout = d }),
	"TPMEM_HTTP_IDLE_TIMEOUT":                 durationSetter("http.idle_timeout", func(c *Config, d time.Duration) { c.HTTP.IdleTimeout = d }),
	"TPMEM_HTTP_SHUTDOWN_TIMEOUT":             durationSetter("http.shutdown_timeout", func(c *Config, d time.Duration) { c.HTTP.ShutdownTimeout = d }),
	"TPMEM_HTTP_MAX_HEADER_BYTES":             intSetter("http.max_header_bytes", 0, func(c *Config, v int64) { c.HTTP.MaxHeaderBytes = int(v) }),
	"TPMEM_HTTP_MAX_BODY_BYTES":               intSetter("http.max_body_bytes", 64, func(c *Config, v int64) { c.HTTP.MaxBodyBytes = v }),
	"TPMEM_DATABASE_URL":                      secretSetter("database.url", func(c *Config, r SecretRef) { c.Database.URL = r }),
	"TPMEM_DATABASE_CONNECT_TIMEOUT":          durationSetter("database.connect_timeout", func(c *Config, d time.Duration) { c.Database.ConnectTimeout = d }),
	"TPMEM_DATABASE_HEALTH_TIMEOUT":           durationSetter("database.health_timeout", func(c *Config, d time.Duration) { c.Database.HealthTimeout = d }),
	"TPMEM_DATABASE_STATEMENT_TIMEOUT":        durationSetter("database.statement_timeout", func(c *Config, d time.Duration) { c.Database.StatementTimeout = d }),
	"TPMEM_DATABASE_LOCK_TIMEOUT":             durationSetter("database.lock_timeout", func(c *Config, d time.Duration) { c.Database.LockTimeout = d }),
	"TPMEM_DATABASE_MAX_CONNECTION_LIFETIME":  durationSetter("database.max_connection_lifetime", func(c *Config, d time.Duration) { c.Database.MaxConnectionLifetime = d }),
	"TPMEM_DATABASE_MAX_CONNECTION_IDLE_TIME": durationSetter("database.max_connection_idle_time", func(c *Config, d time.Duration) { c.Database.MaxConnectionIdleTime = d }),
	"TPMEM_DATABASE_MIN_CONNECTIONS":          intSetter("database.min_connections", 32, func(c *Config, v int64) { c.Database.MinConnections = int32(v) }),
	"TPMEM_DATABASE_MAX_CONNECTIONS":          intSetter("database.max_connections", 32, func(c *Config, v int64) { c.Database.MaxConnections = int32(v) }),
	"TPMEM_LOG_LEVEL":                         func(c *Config, v string) error { c.Log.Level = v; return nil },
	"TPMEM_TELEMETRY_MODE":                    func(c *Config, v string) error { c.Telemetry.Mode = v; return nil },
	"TPMEM_TELEMETRY_ENDPOINT":                func(c *Config, v string) error { c.Telemetry.Endpoint = v; return nil },
	"TPMEM_TELEMETRY_SERVICE_NAME":            func(c *Config, v string) error { c.Telemetry.ServiceName = v; return nil },
	"TPMEM_TELEMETRY_SAMPLE_RATIO": func(c *Config, v string) error {
...
func applyEnvironment(c *Config, environ []string) error {
	...
	for _, item := range environ {
		...
		if !found || !strings.HasPrefix(name, "TPMEM_") {
			continue
		}
		set, ok := environmentSetters[name]
		if !ok {
			return fmt.Errorf("unknown ThinkPixelMEM environment variable %s", name)
		}
		if seen[name] {
...
func applyFlags(c *Config, args []string) error {
	set := flag.NewFlagSet("thinkpixelmem", flag.ContinueOnError)
	set.SetOutput(io.Discard)
	set.String("config", "", "configuration file")
	for envName, apply := range environmentSetters {
		name := strings.ToLower(strings.ReplaceAll(strings.TrimPrefix(envName, "TPMEM_"), "_", "-"))
